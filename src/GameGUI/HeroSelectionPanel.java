...
public class HeroSelectionPanel extends JPanel {
    ...
    private static class HeroCard extends JPanel {
        ...
        HeroCard(HeroDefinition def) {
            ...
            // Use null layout so sprite floats on top of the placeholder area
            ...
            if (def.name.equals("Kael Saint Laurent")) {
                // Animated sprite label — centered, fills the top panel
                JLabel spriteLabel = new JLabel();
                spriteLabel.setHorizontalAlignment(SwingConstants.CENTER);
                spriteLabel.setVerticalAlignment(SwingConstants.CENTER);
                // Position fills entire top panel; sprite is centered inside
                spriteLabel.setBounds(0, 0, 400, 120); // wide enough; actual card width set at runtime
                ...
                if (sheetUrl != null) {
                    try {
                        ...
                        // Single row sheet: 1056x125, 8 frames of 132x125
                        int FRAMES = 8;
                        int FRAME_W = sheet.getWidth() / FRAMES; // 132
                        int FRAME_H = sheet.getHeight();          // 125
                        int DISPLAY_W = FRAME_W;  // natural size
                        int DISPLAY_H = FRAME_H;

                        ImageIcon[] icons = new ImageIcon[FRAMES];
                        for (int i = 0; i < FRAMES; i++) {
                            BufferedImage crop = sheet.getSubimage(i * FRAME_W, 0, FRAME_W, FRAME_H);
                            // Remove black background — make pure black pixels transparent
                            ...
                            for (int x = 0; x < FRAME_W; x++) {
                                for (int y = 0; y < FRAME_H; y++) {
                                    ...
                                    int g = (px >> 8)  & 0xFF;
                                    int b =  px        & 0xFF;
                                    // Treat near-black pixels as transparent (threshold 30)
                                    if (r < 30 && g < 30 && b < 30) {
                        ...
                        spriteLabel.setIcon(icons[6]); // start on 2nd to last frame
                        int[] frameIdx = {6};
                ...
                // Re-center sprite label whenever the panel is resized
                ...
                if (sheetUrl != null) {
                    try {
                        ...
                        System.out.println("[KarlSprite] Sheet: " + sheet.getWidth() + "x" + sheet.getHeight()
                        ...
                        spriteLabel.setText(def.emoji);
                        spriteLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 48));
                        System.out.println("[KarlSprite] Error: " + ex.getMessage());
                    }
                } else {
                    spriteLabel.setText(def.emoji);
                    spriteLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 48));
                    System.out.println("[KarlSprite] Missing asset");
                }
                ...
                if (sheetUrl != null) {
                    try {
                        ...
                        spriteLabel.setText(def.emoji);
                        spriteLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 48));
                        System.out.println("[SimonSprite] Error: " + ex.getMessage());
                    }
                } else {
                    spriteLabel.setText(def.emoji);
                    spriteLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 48));
                    System.out.println("[SimonSprite] Missing asset");
                }
                ...
                if (sheetUrl != null) {
                    try {
                        ...
                        spriteLabel.setText(def.emoji);
                        spriteLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 48));
                        System.out.println("[NullSprite] Error: " + ex.getMessage());
                    }
                } else {
                    spriteLabel.setText(def.emoji);
                    spriteLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 48));
                    System.out.println("[NullSprite] Missing asset");
                }
